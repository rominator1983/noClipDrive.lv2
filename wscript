...
def build(bld):
    ...
    # Copy other data files to build bundle (build/noClipDrive.lv2)
    for i in ['noClipDrive.ttl']:
        bld(features     = 'subst',
            is_copy      = True,
            source       = i,
    ...
    obj = bld(features     = 'c cshlib lv2lib',
              source       = 'noClipDrive.c',
              name         = 'noClipDrive',
              target       = 'lv2/%s/noClipDrive' % bundle,
              install_path = '${LV2DIR}/%s' % bundle,
              uselib       = 'M LV2')
